CLASS zcl_ce_rapsalesdivision DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    TYPES t_division_range TYPE RANGE OF ZRAP_SalesDivision_ES5-Agency_Id.
    TYPES t_business_data TYPE TABLE OF ZRAP_SalesDivision_ES5.

    METHODS get_divisions
      IMPORTING
...
CLASS zcl_ce_rapsalesdivision IMPLEMENTATION.
...
*    http_client = cl_web_http_client_manager=>create_by_http_destination( i_destination = http_destination ).
    ...
    service_consumption_name = to_upper( 'ZSC_DIVISION' ).
