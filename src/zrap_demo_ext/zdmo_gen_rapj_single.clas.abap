    CLASS zdmo_gen_rapj_single DEFINITION
...
**************************************************************************
  ...
  PRIVATE SECTION.
    ...
    CONSTANTS:
      co_prefix                     TYPE string           VALUE 'ZRAP_DEMO_DEVELOPER_',
      co_entity_name                TYPE string           VALUE 'Shop',
      co_session_name               TYPE string           VALUE 'RAPJ',
...
CLASS ZDMO_GEN_RAPJ_SINGLE IMPLEMENTATION.
  ...
  METHOD create_extension_package.
    ...
    lo_specification->set_short_description( |RAP { co_session_name }  Developer Extensibility Demo| ).
  ...
  METHOD create_rap_bo.
    ...
    DATA bgpf_operation TYPE REF TO zdmo_cl_rap_generator_bgpf .
    bgpf_operation = NEW zdmo_cl_rap_generator_bgpf(
...
*    DATA(ls_package) = xco_cp_abap_repository=>package->for( co_zlocal_package )->read( ).
      ...
      DATA(lo_transport_request) = xco_cp_cts=>transports->workbench( lv_transport_target )->create_request( 'ABAP Cloud for dev ext project' ).
...
|       \{\r\n| &
|           "alias": "Product",\r\n| &
|           "name": "ZRAPJI_VH_Product_{ unique_suffix }",\r\n| &
...
*    transport = 'D23K900976'. " <-- maintain your transport request here
    ...
    data_generator_class_name     = |zrapJ_cl_vh_product_{ unique_suffix }|.
...
*        generate_behavior_impl_class( transport ).
  ...
  METHOD generate_vh_custom_entity.
    ...
    DATA(lo_put_operation) = get_put_operation( vh_environment ).             ""ZRAPJI_VH_Product_{ unique_suffix }"
    DATA(lo_interface_specification) = lo_put_operation->for-ddls->add_object( |ZRAPJI_VH_Product_{ unique_suffix }|
...
*    DATA(lo_put_operation) = get_put_operation( package_environment ).
    ...
    lo_public_section->add_type( 't_products'
        ...
        io_row_type = xco_cp_abap_dictionary=>table_type( |ZRAPJI_VH_Product_{ unique_suffix }| )
  ...
  ( |   ). | )
    ...
    lo_specification->implementation->add_method( |if_rap_query_provider~select|
        ...
        ( |  DATA business_data TYPE TABLE OF ZRAPJI_VH_Product_{ unique_suffix }.| )
        ( |  DATA business_data_line TYPE ZRAPJI_VH_Product_{ unique_suffix } .       | )
