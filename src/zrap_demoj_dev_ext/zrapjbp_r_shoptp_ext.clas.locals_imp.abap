...
CLASS LHC_SHOP IMPLEMENTATION.
  ...
  SELECT MAX( ORDER_ID ) FROM ZRAP_DEMOJ_DEV_A INTO @DATA(max_object_id).
  "Add support for draft if used in modify
  "SELECT SINGLE FROM FROM ZRAPJSHOP00D_EXT FIELDS MAX( OrderID ) INTO @DATA(max_orderid_draft). "draft table
